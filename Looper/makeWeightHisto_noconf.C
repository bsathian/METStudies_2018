...
void makeWeightHisto_noconf(TString output_location, TString infile1, TString infile2, TString hist1, TString hist2, TString output_hist_name, vector<int> binRange)
{
  ...
  if (f_primary && f_secondary) {
    h_primary = (TH1D*)f_secondary->Get(hist2)->Clone("h1_"+hist1);
    h_secondary = (TH1D*)f_secondary->Get(hist2)->Clone("h2_"+hist2);
    //h_primary = (TH1D*)h_secondary->Clone("h1_"+hist1);
    h_primary_temp = (TH1D*)f_primary->Get(hist1)->Clone("htemp_"+hist1);
    for (int i = binRange[0]; i <= binRange[1]; i++) {
      h_primary->SetBinContent(i, h_primary_temp->GetBinContent(i));
    }
    h_primary->SetBinContent(binRange[1]+1, h_primary_temp->Integral(binRange[1]+1, h_primary_temp->GetSize()-1));
    //h_primary->GetXaxis()->SetRange(binRange[0], binRange[1]);
    //h_secondary->GetXaxis()->SetRange(binRange[0], binRange[1]);
  }
  else{
    cout<<"Error, could not open baseline files, please check they exist where specified and try again"<<endl;
    exit(1);
  ...
  h_primary_scaled=(TH1D*) h_primary->Clone(hist1+"_scaled1");
  h_secondary_scaled=(TH1D*) h_secondary->Clone(hist2+"_scaled2");
  h_secondary_scaled->Scale(1./h_secondary->GetSumOfWeights());
  h_primary_scaled->Scale(1./h_primary->GetSumOfWeights());
